###################################
# Script : 
# 1) To generate system 
# specific information from mmcif file
# 2) Contains master class 
#
...
def dict_to_JSlist_rows(d1,d2):
    L=[]
    L.append(['Chain ID','Rigid bodies','Flexible regions'])
    for i,j in d1.items():
...
class get_input_information(object):
    ...
    def get_dataset_xl_info(self,id):
        ...
        lt='Linker name: %s, Number of cross-links: %s' % (restraints['Restraint info'][restraints['Dataset ID'].index(id)][0],restraints['Restraint info'][restraints['Dataset ID'].index(id)][1])
        return (lt)
    ...
    def get_ensembles(self):
        ...
        if len(s)>0:
            ensemble_comp={'Ensemble number':[],'Ensemble name':[],'Representative model ID':[],'Number of models':[],
            ...
            for i in s:
                ...
                ensemble_comp['Representative model ID'].append(str(i.model_group._id))
    ...
    def get_restraints(self):
        ...
        for j,i in enumerate(r):
            ...
            if 'CrossLink' in str(i.__class__.__name__):
                restraints_comp['Restraint info'].append((i.linker.auth_name,str(len(i.experimental_cross_links))+' cross-links'))
            if 'EM3D' in str(i.__class__.__name__):
                restraints_comp['Restraint info'].append((i.fitting_method,i.number_of_gaussians))
