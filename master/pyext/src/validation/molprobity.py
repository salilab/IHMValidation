...
class get_molprobity_information(get_input_information):
    ...
    def process_clash(self, line: list) -> dict:
        ...
        for ind in range(0, len(clashes.keys())):
            output_line = [j for k, j in enumerate(
                line) if k > int(count[i]) and k < int(count[i+1])]
    ...
    def molprobity_detailed_table_bonds(self, bonds: list) -> dict:
        ...
        f_mp_D = open(os.path.join(os.getcwd(), self.resultpath,
                                   self.ID+'_molprobity_bond.txt'), 'w+')
        if len(bonds) > 0:
            bondtype_diff = {}
            bondtype_dist = {}
            bondtype_ex = {}
    ...
    def molprobity_detailed_table_angles(self, angles: list) -> dict:
        ...
        f_mp_A = open(os.path.join(os.getcwd(), self.resultpath,
                                   self.ID+'_molprobity_angles.txt'), 'w+')
        if len(angles) > 0:
            angle_diff = {}
            angle_dist = {}
            angle_ex = {}
            dict1 = {'Angle type': [], 'Observed angle (&#176)': [], 'Ideal angle (&#176)': [
            ], 'Number of outliers': []}
