...
class get_excluded_volume(get_input_information):
    ...
    def get_xyzr(self, spheres: pd.DataFrame) -> pd.DataFrame:
        ...
        model_spheres = {i+1: [j.x, j.y, j.z, j.radius]
                         for i, j in enumerate(spheres)}
        model_spheres_df = pd.DataFrame(
            model_spheres, index=['X', 'Y', 'Z', 'R'])
        return model_spheres_df

    def get_xyzr_complete(self, model_ID: int, spheres: list) -> pd.DataFrame:
        """ get X,Y,Z,R, chain and model ID from sphere objects"""

        model_spheres = {i+1: [j.x, j.y, j.z, j.radius,
                               j.asym_unit._id, model_ID] for i, j in enumerate(spheres)}
        model_spheres_df = pd.DataFrame(
            model_spheres, index=['X', 'Y', 'Z', 'R', 'Chain_ID', 'Model_ID'])
        return model_spheres_df
    ...
    def get_exc_vol_for_models(self, model_dict: dict) -> dict:
        ...
        f_exv = open(os.path.join(
            os.getcwd(), 'static/results/', self.ID+'exv.txt'), 'w+')
    ...
    def get_exc_vol_for_models_normalized(self, model_dict: dict) -> dict:
        ...
        for indx, model in model_dict.items():
            excluded_volume['Models'].append(indx)
            df = self.get_xyzr(model)
            satisfaction = self.get_violation_normalized(
    ...
    def run_exc_vol_parallel(self, model_dict: dict) -> dict:
        ...
        excluded_volume = {'Models': list(model_dict.keys()), 'Excluded Volume Satisfaction': [
            i[0] for i in complete_list], 'Number of violations': [i[1] for i in complete_list]}
        f_exv = open(os.path.join(
            os.getcwd(), '../static/results/', self.ID+'exv.txt'), 'w+')
        return excluded_volume
