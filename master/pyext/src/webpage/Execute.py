...
template_pdf = "template_pdf2.html"
...
def write_pdf(mmcif_file,Template_Dict, template_file,dirName,dirName_Output):
    ...
    pdfkit.from_file(os.path.join(os.path.join(dirName,validation.utility.get_output_file_temp_html(mmcif_file))), 
        os.path.join(os.path.join(dirName_Output,validation.utility.get_output_file_pdf(mmcif_file))),
       options=options)
...
def write_supplementary_table(mmcif_file,Template_Dict,template_file,dirName,dirName_supp):
    template = templateEnv.get_template(template_file)
    outputText=template.render(Template_Dict)
    with open(os.path.join(os.path.join(dirName,validation.utility.get_supp_file_html(mmcif_file))),"w") as fh:
        fh.write(outputText)
    pdfkit.from_file(os.path.join(os.path.join(dirName,validation.utility.get_supp_file_html(mmcif_file))), 
...
if __name__ == "__main__":
    ...
    #write_pdf(args.f,template_dict,template_pdf,dirName,dirName_Output)
    template_dict=report.run_supplementary_table(template_dict,
                                                ...
                                                clustering=args.c,
                                                resolution=args.res)
    write_supplementary_table(args.f,template_dict,template_file_supp,dirName,dirName_Output)
